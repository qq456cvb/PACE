...
def visualize_scenes():
    ...
    rel_extrinsics1 = []
    rel_extrinsics2 = []
    for img_id in tqdm(range(0, len(list((Path(root) / 'cam0/rgb_marker').glob('*.png'))), 10)):
        scene_pcs = []
        for i in range(3):
    ...
    for root in Path('data/videos/scene_2').glob('video_7'):
        ...
        for img_id in tqdm(range(0, len(list((root / 'cam0/rgb_marker').glob('*.png'))), 10)):
            ...
            for j in range(1, 3):
                ...
                #     'Matches: {}'.format(np.sum(valid0)),
                # ]

                # color = cm.jet(mconf)
                # make_matching_plot_fast((pred_sp['image0'][0, 0].cpu().numpy() * 255.).astype(np.uint8), 
                #                         (pred_sp['image'+str(j)][0, 0].cpu().numpy() * 255.).astype(np.uint8),
                #                         kpts0, kpts1, kpts0[triplet_matches[0][valid0]], kpts1[triplet_matches[j][valid0]],
                #                         color, text, None, False, 10,
                #                         True, '', '')
            Corresp.append(np.concatenate([pred_sp[f'keypoints{i}'][0].cpu().numpy()[triplet_matches[i][valid0]] for i in range(3)], -1).T)
